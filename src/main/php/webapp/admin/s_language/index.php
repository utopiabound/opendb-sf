...
function generate_language_sql($language, $options = NULL)
{
	...
	{
		$sqlscript ='#########################################################'.
					$CRLF.'# OpenDb '.get_opendb_version().' \''.$language.'\' Language Pack'.
					$CRLF.'#########################################################'.$CRLF.$CRLF;
					
		$sqlscript .= "INSERT INTO s_language (language, description, default_ind) ".
						"VALUES ('".$language_r['language']."', '".addslashes($language_r['description'])."', '".$language_r['default_ind']."'); ".$CRLF;
										
		$results = fetch_language_langvar_rs($language, $options);
		if($results)
		{
			$sqlscript .= $CRLF.'#'.
						$CRLF.'# System Language Variables'.
						$CRLF.'#'.
						$CRLF;
			...
			while($lang_var_r = db_fetch_assoc($results))
			{
				...
				$sqlscript .= "INSERT INTO s_language_var (language, varname, value) ".
						"VALUES ('".$language_r['language']."', '".$lang_var_r['varname']."', '".addslashes($value)."'); ".$CRLF;
			}
		...
		if($language_r['default_ind'] != 'Y')
		{
			...
			{
				$sqlscript .= $CRLF.'#'.
							$CRLF.'# System Table Language Variables'.
							$CRLF.'#'.
							$CRLF;
						...
						while(list(, $column) = each($tableconf_r['columns']))
						{
							...
							if($results)
							{
								while($lang_var_r = db_fetch_assoc($results))
								{
									...
									if(strlen($value))
									{
										$sqlscript .= "INSERT INTO s_table_language_var (language, tablename, columnname, key1, key2, key3, value) ".
												"VALUES ('".$language_r['language']."', '".$table."', '".$column."', '".$lang_var_r['key1']."', ".(strlen($lang_var_r['key2'])>0?"'".$lang_var_r['key2']."'":"''").", ".(strlen($lang_var_r['key3'])>0?"'".$lang_var_r['key3']."'":"''").", '".addslashes($value)."'); ".$CRLF;
									}
								}
								db_free_result($results);
							}
...
function build_table_page($language)
{
	...
	$block .= "<div class=\"tabContainer\"><form name=\"config\" action=\"$PHP_SELF\" method=\"POST\">".
		"<input type=\"hidden\" name=\"type\" value=\"".$ADMIN_TYPE."\">".
		"<input type=\"hidden\" name=\"op\" value=\"update-tables\">".
		"<input type=\"hidden\" name=\"language\" value=\"".$language."\">";
