...
function get_list_item_types($s_item_type_group, $s_item_type=NULL)
{
	...
	if($results)
	{
		...
		{
			$types[] = array(
				'value'=>$itemtype_r['s_item_type'],
				'display'=>$itemtype_r['description']);
		}
...
function get_list_item_type_groups()
{
	...
	if($results)
    {
		...
		while($item_type_group_r = db_fetch_assoc($results))
		{
			...
			{
				$item_type_group_rs[] = array(
					'value'=>$item_type_group_r['s_item_type_group'],
					'display'=>$item_type_group_r['description']);
			}
		}
		db_free_result($results);
...
{
	reset($v_item_types);
	while(list(,$item_type_r) = each($v_item_types))
	{
...
function find_field_type_column_config($s_field_type, $display_column_config)
{
	...
	for($k=0; $k<count($display_column_config); $k++)
	{
		if($display_column_config[$k]['column_type'] == 's_field_type' &&
				$display_column_config[$k]['s_field_type'] == $s_field_type)
		{
			$idx_of_element = $k;
			break;
		}
...
function find_attribute_type_column_config($s_attribute_type, $display_column_config)
{
	...
	for($k=0; $k<count($display_column_config); $k++)
	{
		if($display_column_config[$k]['column_type'] == 's_attribute_type' &&
				$display_column_config[$k]['s_attribute_type'] == $s_attribute_type)
		{
			$idx_of_element = $k;
			break;
		}
	...
	if(is_array($display_column_config) && is_array($search_column_config))
	{
		...
		for($i=0; $i<count($display_column_config); $i++)
		{
			if($display_column_config[$i]['column_type'] == 's_attribute_type')
			{
				if(strlen($display_column_config[$i]['s_attribute_type'])>0)
				{
					...
					{
						if(strlen($search_column_config[$idx_of_element]['lookup_attribute_val'])>0 ||
								strlen($search_column_config[$idx_of_element]['attribute_val'])>0 ||
								strlen($search_column_config[$idx_of_element]['attr_update_on'])>0 ||
					...
					strlen($search_column_config[$i]['attr_update_on_days'])>0) )
				{
					if(find_attribute_type_column_config($search_column_config[$i]['s_attribute_type'], $display_column_config) == -1)
					{
						$tmp_display_column_config[] = $search_column_config[$i];
							...
							$search_column_config[$i]['s_field_type'] == 'RATING')
			{
		...
		if($index_of_title!=-1)
		{
			...
			for($i=$index_of_title+1; $i<count($display_column_config); $i++)
			{
				...
				if($display_column_config[$i]['column_type'] == 's_field_type' &&
						$display_column_config[$i]['s_field_type'] == 'STATUSTYPE') 
				{
...
function get_search_query_matrix($HTTP_VARS)
{
	...
	if(strlen($HTTP_VARS['category'])>0)
	{
		...
		if(strlen($HTTP_VARS['s_item_type'])>0)
		{
			...
			$searches[] = array(
						prompt=>get_opendb_lang_var('category'),
	...
	{
		$searches[] = array(
						prompt=>get_opendb_lang_var('s_item_type_group'),
	...
	if(strlen($HTTP_VARS['attribute_type'])>0)
	{
		...
		if(is_not_empty_array($attribute_type_r))
		{
			...
			else
			{
				...
				{
					$search = get_item_display_field(
							NULL,
							$attribute_type_r,
							stripslashes($HTTP_VARS['lookup_attribute_val']), 
							FALSE);
				}
	...
	else
	{
		if(strlen($HTTP_VARS['attribute_val'])>0) // specified a search term without attribute type, this is a global search.
		{
			...
			{
				$searches[] = array(
							prompt=>get_opendb_lang_var('attribute_val').' (<em>'.get_match_type(ifempty($HTTP_VARS['attr_match'],'exact')).'</em>)',
							field=>stripslashes($HTTP_VARS['attribute_val']));
			}
			else
			{
				$searches[] = array(
							prompt=>get_opendb_lang_var('attribute_val').' (<em>'.get_match_type(ifempty($HTTP_VARS['attr_match'],'exact')).', '.get_opendb_lang_var('case_sensitive').'</em>)',
							field=>stripslashes($HTTP_VARS['attribute_val']));
			}
		}
	}
	...
	if (is_opendb_valid_session() || is_site_public_access())
	{
		if(is_user_granted_permission(PERM_VIEW_LISTINGS))
		{
			...
			if(get_opendb_config_var('borrow', 'enable')!==FALSE && 
					get_opendb_config_var('listings.multi_borrow', 'enable')!==FALSE &&
						(get_opendb_config_var('listings.multi_borrow', 'reserve_action')!==FALSE ||
							(get_opendb_config_var('borrow', 'reserve_basket')!==FALSE &&
							get_opendb_config_var('listings.multi_borrow', 'basket_action')===TRUE &&
							(get_opendb_config_var('listings.multi_borrow', 'basket_action_if_not_empty_only')!==TRUE ||
					...
					if(is_not_empty_array($matrix_rs))
	 				{
	 					...
	 					{
	 						echo('<dt>'.$matrix_r['prompt'].'</dt>'.
	 							'<dd>'.$matrix_r['field'].'</dd>');
	 					}
						echo('</dl></div>');
	 				}
				}
						...
						if($v_column_display_config_rs[$i]['include_in_listing']!==FALSE)
						{
							...
							else if($v_column_display_config_rs[$i]['column_type'] == 's_field_type')
							{
		                        ...
		                        else if($v_column_display_config_rs[$i]['s_field_type'] == 'CATEGORY')
								{
									$listingObject->addAttrDisplayColumn(
														$item_r, 
														fetch_cached_attribute_type_r($item_r['catia_s_attribute_type']), 
														fetch_attribute_val_r($item_r['item_id'], $item_r['instance_no'], $item_r['catia_s_attribute_type'], $item_r['catia_order_no']));
								}
								else if($v_column_display_config_rs[$i]['s_field_type'] == 'STATUSTYPE')
								{
			...
			if(get_opendb_config_var('listings', 'allow_override_show_item_image')!==FALSE)
			{
				...
				get_opendb_config_var('listings', 'show_item_image')==TRUE?'Y':'N')).
				"</li>");
			}
