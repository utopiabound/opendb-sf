 ...
 */
function create_opendb_user_and_database(
				$db_root_username,
				$db_root_password,
				$hostname,
				$database,
				...
				&$error)
{
	...
	if($link!==FALSE)
	{
		...
		{
			$sqltext = "GRANT ALL PRIVILEGES ON ".$database.".* TO ".
					$username."@".$hostname.
...
{
	$query = "UPDATE s_opendb_release ".
			"SET upgrade_step = ".(is_numeric($step)?"'$step'":"NULL")." ".
			",upgrade_step_part = ".(is_numeric($step_part)?"'$step_part'":"NULL")." ".
	...
	{
		if($rows_affected>0)
		opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($release_version, $step, $step_part));
		return TRUE;
	}
	else
	{
		opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($release_version, $step, $step_part));
		return FALSE;
	}
}
?>