...
class TitleMask
{
	    ...
	    if($index!=-1)
	    {
	    	if(!isset($this->_title_mask_rs[$index]['parsed_display_mask']))
	    	{
			    ...
			    // now merge extra info in now
			   	$this->_title_mask_rs[$index] = array_merge($this->_title_mask_rs[$index], array('parsed_display_mask'=>$display_mask, 'display_mask_elements'=>$display_mask_elements));
	...
	*/
	function _expand_title_mask($item_instance_r, $mask, &$mask_element_rs, $config_var_rs = NULL)
	{
		...
		for($i=0; $i<count($mask_element_rs); $i++)
		{
					...
					case 'theme_img':
						...
						else
						{
							if($mask_element_rs[$i]['value'] === NULL &&
										strlen(($vtitle = $this->_expand_title_mask(array('s_item_type'=>$item_instance_r['s_item_type']), $mask_element_rs[$i]['title_mask'], $mask_element_rs[$i]['title_mask_elements'])))>0)
							{
			...
			{
				$value = $this->_get_mask_variable_value(
								$mask_element_rs[$i],
								$item_instance_r);
			}
		...
		if(is_array($mask_variable) && isset($mask_variable['s_attribute_type']) && isset($mask_variable['option']))
		{
			...
			if(is_numeric($item_instance_r['item_id']))
			{
				...
				else if($mask_variable['option'] == 'display_type' && is_numeric($item_instance_r['item_id']))
				{
					...
					$value = get_item_display_field(
							$item_instance_r,
							$mask_variable,
							fetch_attribute_val($item_instance_r['item_id'], $item_instance_r['instance_no'], $mask_variable['s_attribute_type']),
							FALSE);
				}
	...
	*/
    function _find_title_mask_idx($s_item_type_group, $s_item_type)
	{
	    ...
	    {
	        if(($s_item_type_group==NULL || $this->_title_mask_rs[$i]['s_item_type_group'] == $s_item_type_group) &&
	                    ($s_item_type == NULL || $this->_title_mask_rs[$i]['s_item_type'] == $s_item_type))
			{
			    return $i;
			}
	    }
