...
/**
* check for record in site_plugin table and class in lib/site/ directory.
*/
...
{
	$query = "SELECT site_type, classname, order_no, title, image, description, external_url, items_per_page, more_info_url ".
			"FROM s_site_plugin ".
			"WHERE site_type = '".$site_type."'";
...
{
	$query = "SELECT site_type, order_no, field, description, IFNULL(stlv.value, prompt) AS prompt, field_type, default_value, refresh_mask ".
...
{
	$query = "SELECT site_type, sequence_number, s_item_type_group, s_item_type, variable, s_attribute_type, lookup_attribute_val_restrict_ind ".
...
{
	$query = "SELECT order_no, field, description, IFNULL(stlv.value, prompt) AS prompt, field_type, default_value, refresh_mask ".
			"FROM s_site_plugin_input_field ".
			"LEFT JOIN s_table_language_var stlv
			ON stlv.language = '".get_opendb_site_language()."' AND
			stlv.tablename = 's_site_plugin_input_field' AND
...
{
	$query = "SELECT sequence_number, s_item_type_group, s_item_type, variable, s_attribute_type, lookup_attribute_val_restrict_ind ".
		"FROM s_site_plugin_s_attribute_type_map ".
		"WHERE site_type = '".$site_type."' AND sequence_number = $sequence_number";
...
{
	$query = "SELECT variable, s_attribute_type, lookup_attribute_val_restrict_ind ".
		"FROM s_site_plugin_s_attribute_type_map ".
		"WHERE site_type = '".$site_type."' ";
