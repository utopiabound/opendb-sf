...
function fetch_address_type_r($address_type)
{
	$query = 'SELECT s_address_type,'.
			'ifnull(stlv.value, description) AS description '.
			'FROM s_address_type '.
...
function fetch_addr_attribute_type_rltshp_r($address_type, $attribute_type, $order_no = NULL)
{
	$query = 'SELECT s_address_type,'.
			's_attribute_type,'.
			'order_no,'.
			'IFNULL(stlv.value, prompt) AS prompt '.
...
function fetch_address_type_attribute_type_rs($s_address_type, $mode = 'query', $order_by = FALSE)
{
	$query = 
		"SELECT sadt.s_address_type, ".
				...
				"saatr.order_no, ".
				"if(length(IFNULL(stlv.value,saatr.prompt))>0,IFNULL(stlv.value,saatr.prompt),IFNULL(stlv2.value,sat.prompt)) as prompt, ".
				"sat.display_type, ".
...
function insert_user_address($user_id, $s_address_type, $public_address_ind, $borrow_address_ind, $start_dt = NULL, $end_dt = NULL)
{
	if(strlen($user_id)>0)
	{
		...
		$query = "INSERT INTO user_address (user_id, s_address_type, start_dt, end_dt, public_address_ind, borrow_address_ind)".
				" VALUES ('".$user_id."','".$s_address_type."',".(strlen($start_dt)>0?$start_dt:"now()").", ".(strlen($end_dt)>0?"'".$end_dt."'":"NULL").", '$public_address_ind', '$borrow_address_ind')";
		...
		{
			opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($user_id, $s_address_type, $public_address_ind, $borrow_address_ind, $start_dt, $end_dt));
			return FALSE;
		}
	}
	else
	{
		opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($user_id, $s_address_type, $start_dt, $end_dt));
		return FALSE;
	}		
}
...
function update_user_address($sequence_number, $public_address_ind, $borrow_address_ind)
{
	if(is_numeric($sequence_number))
	{
		...
		$query = "UPDATE user_address SET ".
				"public_address_ind = '".$public_address_ind."'".
				", borrow_address_ind = '".$borrow_address_ind."'".
	...
	if(is_numeric($ua_sequence_number))
	{
		...
		if ($delete)
		{
			if(db_affected_rows() > 0)
				opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($ua_sequence_number, $s_attribute_type, $order_no));
			return TRUE;
		}
		else
		{
			opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($ua_sequence_number, $s_attribute_type, $order_no));
			return FALSE;
		}
	}
	else
