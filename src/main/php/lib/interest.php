...
{
	$query = "SELECT level".
			" FROM user_item_interest i".
			" WHERE i.item_id = $item_id AND i.instance_no = $instance_no AND i.user_id = '$user_id'";
...
// 	opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($item_id, $user_id, $level));
	$query = "INSERT INTO user_item_interest (item_id, instance_no, user_id, level) ".
					"VALUES ('".addslashes($item_id)."', ".
							"'".addslashes($instance_no)."', ".
							"'".addslashes($user_id)."', ".
							"'".addslashes($level)."')";
	...
	{
		opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($item_id, $user_id, $level));
		return FALSE;
	}
}
...
// 	opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($item_id, $user_id, $level));
	$query = "UPDATE user_item_interest SET ".
	...
	if($level == 0){
		$query = "DELETE FROM user_item_interest".
				" WHERE item_id = '$item_id'".
				" AND instance_no = '$instance_no'";
				" AND user_id = '$user_id'";
	}
