...
function is_exists_site_item_attribute($site_type, $item_id, $instance_no)
{
	$query = "SELECT 'X' ".
			...
			"(ia.instance_no = 0 OR ia.instance_no = '".$instance_no."') AND ".
...
function fetch_value_match_attribute_type_lookup_rs($s_attribute_type, $value_array, $order_by='value', $order='asc')
{
	$query = "SELECT satl.value, IF(LENGTH(IFNULL(stlv.value, display))>0,IFNULL(stlv.value, display),satl.value) AS display, img, checked_ind ".
			...
			stlv.key1 = s_attribute_type AND
			stlv.key2 = satl.value 
			WHERE satl.s_attribute_type = '".$s_attribute_type."' AND satl.value IN (".format_sql_in_clause($value_array).") ".
			"ORDER BY satl.order_no, $order_by $order";
...
{
	return in_array($field_type, array('TITLE', 'ITEM_ID', 'IMAGE', 'CATEGORY', 'DURATION', 'STATUSTYPE', 'STATUSCMNT', 'RATING'));
}
...
{
	$query = "SELECT IF(LENGTH(IFNULL(stlv2.value, siat.prompt))>0,IFNULL(stlv2.value, siat.prompt),IFNULL(stlv.value, sat.prompt)) AS prompt ".
			"FROM (s_item_attribute_type siat,".
			"s_attribute_type sat) ".
			"LEFT JOIN s_table_language_var stlv ON
				stlv.language = '".get_opendb_site_language()."' AND
...
{
	$query =
		"SELECT sat.s_attribute_type,".
				"IFNULL(stlv.value, sat.prompt) AS prompt,".
				"IFNULL(stlv2.value, sat.description) AS description,". 
                "sat.display_type, ".
				"sat.display_type_arg1,".
				"sat.display_type_arg2,".
				"sat.display_type_arg3,".
		...
		"LEFT JOIN s_table_language_var stlv2 ON
				...
				stlv2.key1 = sat.s_attribute_type ".
...
{
	$query = "SELECT item_id, instance_no, s_attribute_type, order_no, attribute_no, lookup_attribute_val, attribute_val FROM item_attribute ".
...
function insert_item_attribute($item_id, $instance_no, $s_attribute_type, $order_no, $attribute_no, $lookup_attribute_val, $attribute_val)
{
	...
	{	
		...
		{
			opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($item_id, $instance_no, $s_attribute_type, $order_no, $attribute_no, $lookup_attribute_val, $attribute_val));
			return TRUE;
		}
		else
		{
			opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($item_id, $instance_no, $s_attribute_type, $order_no, $attribute_no, $lookup_attribute_val, $attribute_val));
			return FALSE;
		}
	}
	else
...
function delete_item_attributes($item_id, $instance_no = NULL, $s_attribute_type = NULL, $order_no = NULL)
{
	...
	if ( $delete )// Even if no attributes were deleted, because there were none, this should still return true.
	{
		if(db_affected_rows() > 0)
			opendb_logger(OPENDB_LOG_INFO, __FILE__, __FUNCTION__, NULL, array($item_id, $instance_no, $s_attribute_type, $order_no));
		return TRUE;
	}
	else
	{
		opendb_logger(OPENDB_LOG_ERROR, __FILE__, __FUNCTION__, db_error(), array($item_id, $instance_no, $s_attribute_type, $order_no));
		return FALSE;
	}
}
?>